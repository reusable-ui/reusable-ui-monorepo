 ...
 * ```ts
 * const definition = paragraphConfigVarDefs.boxShadow; // Resolves to [[ "0px", "0px", "0px", "calc(", "var(--p-marginBlockStart)", " / 4)", "gray" ]]
 * ```
 * 
 * #### **Assigning a Custom Value (Setter)**
 * **Direct Assignment:**
 * ```ts
 * paragraphConfigVarDefs.fontWeightCustom = 900; // Generates "--p-fontWeightCustom: 900;"
 * ```
 * 
 * **Definition Assignment:**
 * ```ts
 * paragraphConfigVarDefs.boxShadow = [[
 *    "0px", "0px", "0px", "calc(", paragraphConfigVars.marginBlockStart, " / 4)", "gray"
 * ]]; // Generates "--p-boxShadow: 0px 0px 0px calc(var(--p-marginBlockStart) / 4) gray;"
 * ```
 * 
 ...
 * When a custom value is assigned using a **valid CSS property name**, it is automatically applied within the styling stylesheet:
 * ```ts
 * paragraphConfigVarDefs.padding = '1rem';
 * ```
 * This generates the following styles:
 * 
 * **Variables stylesheet:**
 ...
 * However, if the property name **is not a recognized CSS property**, it still generates a CSS variable but does not apply automatically:
 * ```ts
 * paragraphConfigVarDefs.booh = 1234;
 * ```
 * This generates:
 * 
 * **Variables stylesheet:**
 ...
 * A variable can be removed using any of the following:
 * ```ts
 * delete paragraphConfigVarDefs.fontWeightCustom;
 * paragraphConfigVarDefs.fontWeightCustom = null;
 * paragraphConfigVarDefs.fontWeightCustom = undefined;
 * ```
 * 
 * #### **Definition Handling**
 * The **cssfn library** processes:
 ...
 * ```
 */
export const paragraphConfigVarDefs    = config[1];
