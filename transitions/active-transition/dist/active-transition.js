...
cssVars, switchOf, } from '@cssfn/core'; // Writes css in javascript.
// Reusable-ui configs:
import { colorParamVars, } from '@reusable-ui/colors'; // A flexible and themeable color management system for web components, utilizing CSS custom properties to enable dynamic styling and easy customization.
// Reusable-ui variants:
import { usesOutlineVariant, } from '@reusable-ui/outline-variant'; // A utility for managing visual outline consistently across React components.
import { usesMildVariant, } from '@reusable-ui/mild-variant'; // A utility for managing mild styling (reading friendly) consistently across React components.
// Reusable-ui features:
import { usesBackgroundFeature, } from '@reusable-ui/background-feature'; // A styling utility for resolving the appropriate background color based on the currently active variants — including theme, emphasize, outline, mild, and bare.
import { usesForegroundFeature, } from '@reusable-ui/foreground-feature'; // A styling utility for resolving the appropriate foreground color based on the currently active variants — including theme, outline, and mild.
import { usesDecorationFeature, } from '@reusable-ui/decoration-feature'; // A styling utility for resolving the appropriate decoration color based on the currently active variants — including theme, outline, and mild.
import { usesBorderFeature, } from '@reusable-ui/border-feature'; // A styling utility for resolving the appropriate border color, geometry, and radius based on the currently active variants — including theme, outline, mild, and bare.
import { filterRegistry } from '@reusable-ui/filter-feature'; // A styling utility for composing a unified filter stack from custom and registered state packages.
// Reusable-ui states:
import { usesActiveState, } from '@reusable-ui/active-state'; // Lifecycle-aware activation state with transition animations and semantic styling hooks for UI components.
/**
 ...
 */
const [activeTransitionVars] = cssVars({ prefix: 'act', minify: false });
 ...
 */
export const usesActiveTransition = (options) => {
    // Extract options and assign defaults:
    const { activeBrightness = 1, activeContrast = 1, activeSaturate = 1, } = options ?? {};
    // Configs:
    ...
    return {
        activeTransitionRule: () => style({
            ...
            ].map(({ regularColor, variantColor, overrideVar }) => vars({
                /**
                 * Color override:
