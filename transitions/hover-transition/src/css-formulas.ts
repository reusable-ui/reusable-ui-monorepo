...
export function absoluteValue(value: CssNumeric | CssAngle | CssLength): CssNumericFormula | CssAngleFormula | CssLengthFormula { return `max((${value}), -(${value}))` }
...
export function ifNegativeWithUnit(condition: CssAngle | CssLength, appliedFactor: CssNumeric): CssAngleFormula | CssLengthFormula { return `(${ensureBetweenZeroAndOne(`-sign(${condition})`)} * (${appliedFactor}))` }
