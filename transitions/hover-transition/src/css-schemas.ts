...
export interface FilterSchemaBase<
    TType        extends 'ratio' | 'angle' | 'length' | 'color',
...
export type FilterSchema =
    ...
    | (FilterSchemaBase<
        ...
        CssLength,                           // Direction value type (blur/length factor for interpolation).
 ...
 * 
 * Covers both:
 * - Standard filter functions (`opacity(...)`, `blur(...)`, `hue-rotate(...)`, `color-mix(...)`).
 ...
 * 
 * Available mappings:
 * - `0` → interpolate relative to zero (e.g. hue-rotate, blur).
 ...
 * 
 * - Uses the current condition context (hover factor, mode).
 * - Normalizes target values based on input type (ratio, angle, length, color).
 ...
 */
export function schemaToCssFormula(this: ConditionContext, schema: FilterSchema): CssFormulaResult {
    // Extract context:
    const {
        hoverFactorCond,
        mode,
    ...
    const directionAwareFactor = ((): CssNumericFormula => {
        // Base factors:
        const fadeInFactor  : CssNumeric        = hoverFactorCond;
        const fadeOutFactor : CssNumericFormula = reverseFactor(hoverFactorCond);
        ...
        switch (inputType) {
            ...
            case 'length':
            case 'color':
                // Angle inputs (hue-rotate), length inputs (blur), and color inputs:
                // - Direction value is the same as `targetValue` (for angle and length) or `blurRadius` (for color).
    ...
    // - May be adjusted for light/dark mode (numeric/angle/length only).
    const interpolatedTarget = ((): CssNumericFormula | CssAngleFormula | CssLengthFormula | CssColorFormula => {
        switch (inputType) {
            ...
            case 'length': {
                ...
                const lightModeTarget : CssNumericFormula | CssAngleFormula | CssLengthFormula = absoluteValue(targetValue satisfies CssNumeric | CssAngle | CssLength);
    ...
    const safeInterpolatedTarget = ((): CssNumericFormula | CssAngleFormula | CssLengthFormula | CssColorFormula => {
        ...
        // - Angle and color schemas never specify limits.
