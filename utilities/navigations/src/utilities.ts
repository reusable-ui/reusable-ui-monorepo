 ...
 */
export const extractPathnameFromSource = (pathSource: string | UrlObject | To): string => {
    // Extract pathname from a string URL, remove the query and hash parts:
    if (typeof pathSource === 'string') return parsePath(pathSource).pathname ?? '';
 ...
 */
export const resolveAbsolutePathFromRelative = (currentPathname: string | string[], relativePath: string): `/${string}` => {
    // Handle absolute path:
    if (relativePath.startsWith('/')) return (relativePath.replace(/\/+$/, '') as `/${string}`) || '/'; // Remove one or more trailing slashes at the end.
 ...
 * @returns The resolved absolute pathname string (always starting with '/').
 */
export const resolveAbsolutePathFromSource = (currentPathname: string | string[], pathSource: string | UrlObject | To): `/${string}` => {
 ...
 */
export const evaluatePathMatch = (actualPathname: string, expectedPathname: string, strategy: PathMatchStrategy): boolean => {
    // Handle non-absolute path:
    if (!actualPathname.startsWith('/') || !expectedPathname.startsWith('/')) return false;
    
    
    
    // Handle exact match:
    if (actualPathname === expectedPathname) return true; // Always match for all strategies.
    ...
    if (Math.abs(lengthDiff) === 1) {
        ...
        if (longerPath.endsWith('/') && longerPath.startsWith(shorterPath)) return true;
    } // if
    ...
    if (strategy === 'partial') {
        ...
        if (!actualPathname.startsWith(expectedPathname)) return false;
        
        
        
        // Ensure the remainder is chopped at segment boundary:
        const boundaryIndex = (
            expectedPathname.endsWith('/')
