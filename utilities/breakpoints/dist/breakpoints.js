import { 
// rules:
rule, alwaysRule, neverRule, } from '@cssfn/cssfn'; // writes css in javascript
...
export const next = (breakpointName) => {
    ...
    if (foundIndex < 0)
        throw TypeError(`Breakpoint '${breakpointName}' is not found in breakpoints.`); // invalid key
    foundIndex++; // move to the next
    if (foundIndex >= sortedBreakpoints.length)
        return null; // there is no breakpoint bigger than `breakpointName`
 ...
 * @throws The specified `breakpointName` is not found in breakpoints.
 */
export const maxBefore = (breakpointName) => {
    const value = min(breakpointName);
    if (value === null)
        return null; // nothing smaller than 0
    if ((value >= 0.02))
        return (value - 0.02);
 ...
 * @throws The specified `breakpointName` is not found in breakpoints.
 */
export const ifScreenWidthAtLeast = (breakpointName, styles) => {
    const minWidth = min(breakpointName);
    if (minWidth === null)
        return alwaysRule(styles); // no minimum limit => always applied the `styles`
 ...
 * @throws The specified `breakpointName` is not found in breakpoints.
 */
export const ifScreenWidthSmallerThan = (breakpointName, styles) => {
    const maxWidthBeforeCurrent = maxBefore(breakpointName);
    if (maxWidthBeforeCurrent === null)
        return neverRule(); // nothing smaller than zero width limit => never apply the `styles`
 ...
 */
export const ifScreenWidthBetween = (lowerBreakpointName, upperBreakpointName, styles) => {
 ...
 * @throws The specified `breakpointName` is not found in breakpoints.
 */
export const ifScreenWidthAt = (breakpointName, styles) => {
    const minWidth = min(breakpointName);
