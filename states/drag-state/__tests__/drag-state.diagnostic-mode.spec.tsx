...
interface DragStateControlledTestCase {
    ...
    updates        : {
         ...
         * - `true`      : set drag to test element
         * - `false`     : set drop to test element
    ...
    for (const { title, dragged : initialDragged, updates } of [
        ...
        */
        ...
        },
        {
            ...
            updates       : [
                {
                    title               : 'Set to initially dragged and wait until the dragging animation finishes',
                    setDrag             : true,
                    
                    delay               : 1500, // wait until the dragging animation finishes
                    expectedDrag        : 'dragged',
                    expectedRunningDrag : 0,
                },
                {
        ...
        test(title, async ({ mount, page }) => {
            ...
            const component = await mount(
                <DragStateTest
                    implementsDragHandlers={true}
                    dragged={initialDragged}
            ...
            for (const { title, setDrag, delay, expectedDrag, expectedRunningDrag} of updates) {
                console.log(`[Subtest] ${title}`);
                
                
                
                // Update the drag state:
                if (setDrag !== undefined) {
                    ...
                    // Simulate user interaction to change the drag state:
                    if (setDrag) {
                        await page.mouse.down(); // simulate drag
                    ...
                    // Wait for the next macrotask since the drop state is a bit deferred than the drag one:
                    if (setDrag === false) {
                        // Wait again if still no event received:
                        for (let attempts = 5; attempts > 0; attempts--) {
                            if (lastNewDrag === undefined) await new Promise<void>((resolve) => {
                    ...
                    expect(lastNewDrag).toBe(setDrag);
                ...
                // Ensure the component is rendered correctly:
                const box2 = component.getByTestId('drag-state-test');
                await expect(box2).toContainText('Drag State Test');
                ...
                if (expectedDrag !== undefined) {
                    expect(box2).toHaveAttribute('data-state', expectedDrag);
                } // if
