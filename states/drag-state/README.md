...
It requires a **press-state (or equivalent)** source to supply `computedDrag` — typically derived from `actualPressed` in `usePressState()` or `useResolvedPressed()`.  
...
### `useResolvedDragged(props, options?)`
