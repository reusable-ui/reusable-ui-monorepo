...
const testCases: ExciteEffectTestCase[] = [
    ...
    ...[false, true].map((reverseIntent) => ({
        ...
        props             : {
            activeFactor  : 0,
            isExcited     : true, // The factor is 0 but going to be 1 when excited, so we set `isExcited` to `true` to trigger the effect application logic.
            reverseIntent,
        },
        expectedFilters   : {
    ...
    ...([-999, -0.5, 1.5, 999] as const).flatMap((factor) =>
        [false, true].map((reverseIntent) => ({
