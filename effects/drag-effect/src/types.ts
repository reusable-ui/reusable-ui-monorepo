...
/**
 * A list of CSS variables used for drag-effect styling.
 * 
 * The keys are used for semantic mapping and documentation purposes. The values are ignored.
 */
export interface DragEffectVars {
    /**
     * References the composed filter applied when the component is transitioning toward dragged/dropped or fully dragged.
     * 
    ...
    /**
     * References the composed transform applied when the component is transitioning toward dragged/dropped or fully dragged.
     * 
     * - Becomes `unset` when the component is fully dropped.
     * - Typically not consumed directly — instead use:
...
/**
 * Configuration options for customizing drag effects.
 */
export interface CssDragEffectOptions
    extends
        // Bases:
        CssFilterEffectOptions
{
     ...
     * to fade *out* on drag, instead of fading *in*.
     * 
    ...
    /**
     * Controls how much the component's opacity is adjusted when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged opacity becomes `0.8 * 0.5 = 0.4`.
     ...
     *   - Instead of fading *in* the opacity adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base opacity is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is inverted when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the dragged state will apply `invert(0.8)` followed by `invert(0.5)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the inversion adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base inversion is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is sepia-toned when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the dragged state will apply `sepia(0.8)` followed by `sepia(0.5)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the sepia adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base sepia effect is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is brightened or darkened when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged brightness becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the brightness adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base brightness is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component's color contrast is adjusted when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged contrast becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the contrast adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base contrast is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component's color saturation is adjusted when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged saturation becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the saturation adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base saturation is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component's color hue is rotated when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged hue rotation becomes `80deg + 50deg = 130deg`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the hue adjustment when dragged, the adjustment fades *out*.
     *   - When fully dropped, the base hue is restored (the effect is fully un-applied).
    ...
    /**
     * Controls how much the component is blurred when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the dragged state will apply `blur(8px)` followed by `blur(5px)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the blur effect when dragged, the effect fades *out*.
     *   - When fully dropped, the base sharpness is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Specifies the drop shadow to apply when fully dragged.
     * 
     * - Transitions smoothly from dropped → dragged.
     ...
     *   the fully dragged drop shadow becomes the combination of both shadows.
...
/**
 * Provides a CSS API for applying drag-state effects that follow the cursor movement,
 * making components **visually carried and repositioned** while being dragged.
 */
export interface CssDragEffect {
    /**
     * Attaches CSS rules for drag-state effects that follow the cursor movement,
     * making components **visually carried and repositioned** while being dragged.
     * 
     ...
     * - factor = 1 → fully dragged (target opacity/brightness/contrast/saturation/etc. applied).
     * - Between 0 and 1 → smooth interpolation between neutral and dragged, applying all configured effects.
     * 
     * Smoothly transitions between drag and drop states by animating filter effects.
     * Affects the entire component surface.
     */
    dragEffectRule : Lazy<CssRule>
     ...
     */
    dragEffectVars : CssVars<DragEffectVars>
}
