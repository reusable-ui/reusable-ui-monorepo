...
import { type DragEffectTestProps, DragEffectTest } from './DragEffectTest.js';
...
interface DragEffectTestCase {
    ...
    /**
     * Props to pass to the `<DragEffectTest>` component.
     */
    props            : DragEffectTestProps
...
const testCases: DragEffectTestCase[] = [
    // Inactive → no effects
    ...
    ...[false, true].map((reverseIntent) => ({
        ...
        props            : {
            activeFactor : 0,
            isDragged    : true, // The factor is 0 but going to be 1 when dragged, so we set `isDragged` to `true` to trigger the effect application logic.
            reverseIntent,
        },
        expectedFilters  : {
    ...
    ...([0.25, 0.5, 0.75] as const).flatMap((factor) =>
        [false, true].map((reverseIntent) => ({
            ...
            exprectedShift   : {
                x            : draggedShiftX * factor,
                y            : draggedShiftY * factor,
            },
        } satisfies DragEffectTestCase)),
    ),
...
test.describe('usesDragEffect', () => {
    for (const { title, props, expectedFilters, exprectedShift } of testCases) {
        test(title, async ({ mount }) => {
            ...
            const box = component.getByTestId('drag-effect-test');
            
            // Verify the component renders correctly:
            await expect(box).toContainText('Drag Effect Test');
            
            // Allow time for stylesheets to fully apply:
            await new Promise((resolve) => setTimeout(resolve, 500));
            
            // Extract computed filter and transform strings from the element:
            const [computedFilterString, transformString] = await box.evaluate((el) => {
            ...
            const filterFunctions = computedFilterString.match(/([a-z-]+\((?:[^()]|\([^()]*\))*\))/gi) || [];
            ...
            // Validate expected shift (x and y):
            if (transformString === 'none') {
                expect(0).toBeCloseTo(exprectedShift.x, 2);
                expect(0).toBeCloseTo(exprectedShift.y, 2);
            }
            else {
                ...
                // Extract translate(x, y) from the `transformArray`:
                const actualShiftX = transformArray[4]; // element at index 4 is translateX
                const actualShiftY = transformArray[5]; // element at index 5 is translateY
                
                // Assert that the actual shifts are close to the expected shifts:
                // - Allowing for minor discrepancies due to rendering differences.
                expect(actualShiftX).toBeCloseTo(exprectedShift.x, 2);
                expect(actualShiftY).toBeCloseTo(exprectedShift.y, 2);
            }
        });
    }
});
