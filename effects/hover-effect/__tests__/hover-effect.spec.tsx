...
interface HoverEffectTestCase {
    ...
    /**
     * Expected text decoration on hover (e.g., `'underline'`),
     * or `'none'` if no text decoration effect is expected.
     */
    expectedTextDecoration : 'underline' | 'none'
}
...
const testCases: HoverEffectTestCase[] = [
    ...
    ...[false, true].map((reverseIntent) => ({
        ...
        expectedTextDecoration : !reverseIntent ? 'none' : 'underline',
    ...
    ...[false, true].map((reverseIntent) => ({
        ...
        props                  : {
            activeFactor       : 0,
            isHovered          : true, // The factor is 0 but going to be 1 when hovered, so we set `isHovered` to `true` to trigger the effect application logic.
            reverseIntent,
        },
        expectedFilters        : {
        ...
        expectedTextDecoration : !reverseIntent ? 'underline' : 'none',
    } satisfies HoverEffectTestCase)),
...
test.describe('usesHoverEffect', () => {
    for (const { title, props, expectedFilters, expectedTextDecoration } of testCases) {
        test(title, async ({ mount }) => {
            ...
            const computedTextDecoration = await box.evaluate((el) => getComputedStyle(el).textDecorationLine);
            expect(computedTextDecoration).toBe(expectedTextDecoration);
        });
    }
});
