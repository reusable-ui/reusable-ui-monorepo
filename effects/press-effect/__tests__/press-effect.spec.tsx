...
import { type PressEffectTestProps, PressEffectTest } from './PressEffectTest.js';
...
interface PressEffectTestCase {
    ...
    /**
     * Props to pass to the `<PressEffectTest>` component.
     */
    props            : PressEffectTestProps
...
const testCases: PressEffectTestCase[] = [
    // Inactive → no effects
    ...
    ...[false, true].map((reverseIntent) => ({
        ...
        props            : {
            activeFactor : 0,
            isPressed    : true, // The factor is 0 but going to be 1 when pressed, so we set `isPressed` to `true` to trigger the effect application logic.
            reverseIntent,
        },
        expectedFilters  : {
...
test.describe('usesPressEffect', () => {
    for (const { title, props, expectedFilters } of testCases) {
        test(title, async ({ mount }) => {
            ...
            const box = component.getByTestId('press-effect-test');
            
            // Verify the component renders correctly:
            await expect(box).toContainText('Press Effect Test');
