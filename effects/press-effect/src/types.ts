...
/**
 * A list of CSS variables used for press-effect styling.
 * 
 * The keys are used for semantic mapping and documentation purposes. The values are ignored.
 */
export interface PressEffectVars {
    /**
     * References the filter applied when the component is transitioning toward pressed/released or fully pressed.
     * 
...
/**
 * Configuration options for customizing press effects.
 */
export interface CssPressEffectOptions
    extends
        // Bases:
        CssFilterEffectOptions
{
     ...
     * to fade *out* on press, instead of fading *in*.
     * 
     * Defaults to `true` (enables reverse intent, allowing negative values to fade *out* the effect on press).
     */
    enablesReverseIntent ?: CssFilterEffectOptions['enablesReverseIntent']
    
    /**
     * Controls how much the component's opacity is adjusted when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed opacity becomes `0.8 * 0.5 = 0.4`.
     ...
     *   - Instead of fading *in* the opacity adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base opacity is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is inverted when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the pressed state will apply `invert(0.8)` followed by `invert(0.5)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the inversion adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base inversion is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is sepia-toned when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the pressed state will apply `sepia(0.8)` followed by `sepia(0.5)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the sepia adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base sepia effect is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component is brightened or darkened when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed brightness becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the brightness adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base brightness is restored (the effect is fully un-applied).
     * 
     * Defaults to `0.6` (fairly darken in light mode, fairly lighten in dark mode).
     */
    brightness           ?: CssFilterEffectOptions['brightness']
    
    /**
     * Controls how much the component's color contrast is adjusted when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed contrast becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the contrast adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base contrast is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component's color saturation is adjusted when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed saturation becomes `0.8 * 0.5 = 0.4`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the saturation adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base saturation is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Controls how much the component's color hue is rotated when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed hue rotation becomes `80deg + 50deg = 130deg`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the hue adjustment when pressed, the adjustment fades *out*.
     *   - When fully released, the base hue is restored (the effect is fully un-applied).
    ...
    /**
     * Controls how much the component is blurred when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the pressed state will apply `blur(8px)` followed by `blur(5px)`.
     ...
     * - Negative values are supported but have a **special meaning**:
     *   - They reverse the interpolation direction.
     *   - Instead of fading *in* the blur effect when pressed, the effect fades *out*.
     *   - When fully released, the base sharpness is restored (the effect is fully un-applied).
     * 
    ...
    /**
     * Specifies the drop shadow to apply when fully pressed.
     * 
     * - Transitions smoothly from released → pressed.
     ...
     *   the fully pressed drop shadow becomes the combination of both shadows.
...
/**
 * Provides a CSS API for applying press-state effects that acknowledge user input,
 * making components **visually confirming command** when pressed (clicked).
 */
export interface CssPressEffect {
    /**
     * Attaches CSS rules for press-state effects that acknowledge user input,
     * making components **visually confirming command** when pressed (clicked).
     * 
     ...
     * - factor = 1 → fully pressed (target opacity/brightness/contrast/saturation/etc. applied).
     * - Between 0 and 1 → smooth interpolation between neutral and pressed, applying all configured effects.
     * 
     * Smoothly transitions between press and release states by animating filter effects.
     * Affects the entire component surface.
     */
    pressEffectRule : Lazy<CssRule>
    
    /**
     * Exposes press-effect CSS variables for transitional effects.
     * 
     * Includes:
     * - `pressFilter` : Opacity, brightness, contrast, saturation, hue-rotate, drop-shadow, blur, and blur radius interpolation during press transitions.
     * 
     * ⚠️ **Caution**: These variables are invalid when the component is fully released.
     ...
     */
    pressEffectVars : CssVars<PressEffectVars>
}
